...
class TokenType:
    ...
    PLUS = "PLUS"
    MINUS = "MINUS"
    MUL = "MUL"         # Додаємо токен для множення
    DIV = "DIV"         # Додаємо токен для ділення
    LPAREN = "LPAREN"   # Токен для відкритої дужки
    RPAREN = "RPAREN"   # Токен для закритої дужки
...
class Lexer:
    ...
    def get_next_token(self):
        ...
        while self.current_char is not None:
            ...
            if self.current_char == "*":
                self.advance()
                return Token(TokenType.MUL, "*")  # Новий токен для множення

            if self.current_char == "/":
                self.advance()
                return Token(TokenType.DIV, "/")  # Новий токен для ділення

            if self.current_char == "(":
                self.advance()
                return Token(TokenType.LPAREN, "(")  # Токен для відкритої дужки

            if self.current_char == ")":
                self.advance()
                return Token(TokenType.RPAREN, ")")  # Токен для закритої дужки
...
class Parser:
    ...
    def factor(self):
        ...
        if token.type == TokenType.INTEGER:
            ...
            node = self.expr()  # Обробляємо вираз всередині дужок
